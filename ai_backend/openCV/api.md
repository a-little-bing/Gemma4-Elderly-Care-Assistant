...
```json
{
  "frame_path": "yolo/inputs/test.jpg"
}
```

`frame_path` supports image paths, video paths, webcam indexes such as `"0"` or `"webcam:0"`, and `data:image/...;base64,...` data URLs. If `frame_path` is omitted, the service tries webcam 0.
    ...
    "detections": [
      {
        ...
        "width": 0.42,
        "height": 0.18,
        "bbox_xyxy": [179.2, 278.4, 448.0, 364.8]
      }
    ]
  }
}
